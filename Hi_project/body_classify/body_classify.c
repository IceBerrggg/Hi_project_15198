...
#define MODEL_FILE_GESTURE    "/userdata/models/hand_classify/six_resent18_detect_inst.wk" // darknet framework wk model
...
static void HandDetectFlag(const RecogNumInfo resBuf)
{
    ...
    switch (resBuf.num) {
        case 0u:
            ...
            SAMPLE_PRT("----body type----:%s\n", gestureName);
            break;
        case 1u:
            gestureName = "medium man";
            UartSendRead(uartFd, ForefingerGesture); 
            SAMPLE_PRT("----body type----:%s\n", gestureName);
            break;
        case 2u:
            gestureName = "thin man";
            UartSendRead(uartFd, OkGesture); 
            SAMPLE_PRT("----body type----:%s\n", gestureName);
            break;
        case 3u:
            gestureName = "fat woman";
            UartSendRead(uartFd, PalmGesture); 
            SAMPLE_PRT("----body type----:%s\n", gestureName);
            break;
        case 4u:
            gestureName = "medium woman";
            UartSendRead(uartFd, YesGesture); 
            SAMPLE_PRT("----body type----:%s\n", gestureName);
            break;
        case 5u:
            gestureName = "thin man";
            UartSendRead(uartFd, ForefingerAndThumbGesture); 
            SAMPLE_PRT("----body type----:%s\n", gestureName);
            break;
        default:
            gestureName = "unknown";
...
HI_S32 Yolo2HandDetectResnetClassifyCal(uintptr_t model, VIDEO_FRAME_INFO_S *srcFrm, VIDEO_FRAME_INFO_S *dstFrm)
{
    ...
    SAMPLE_CHECK_EXPR_RET(ret != HI_SUCCESS, ret, "body detect for YUV Frm to Img FAIL, ret=%#x\n", ret);
