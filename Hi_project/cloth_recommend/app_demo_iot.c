...
static void DeviceConfigInit(hi_gpio_value val)     //初始设备配置
{
    hi_io_set_func(HI_IO_NAME_GPIO_9, HI_IO_FUNC_GPIO_9_GPIO);  //设置IO口复用
    hi_gpio_set_dir(HI_GPIO_IDX_9, HI_GPIO_DIR_OUT);    //
    hi_gpio_set_ouput_val(HI_GPIO_IDX_9, val);      //
}

static int  DeviceMsgCallback(FnMsgCallBack msgCallBack)    //回调函数
{
...
/* publish sample */
hi_void IotPublishSample(hi_uchar buffer)  //向云端发布实例
{
    /* reported attribute */
    WeChatProfile weChatProfile = {
        ...
        /* report temperature */
        .reportAction.subDeviceActionTemperature = buffer,
        .reportAction.temperatureData = "111", /* 30 :temperature data */
        /* report humidity */
        .reportAction.subDeviceActionHumidity = "humidity",
        .reportAction.humidityActionData = "222", /* humidity data */
...
static hi_void *DemoEntry(const char *arg)
{
    ...
    while (1) {
        hi_uchar transbuff = UartTransmit();
        // TaskMsleep(10000);
        IotPublishSample(transbuff); // 发布例程
#endif
...
static void AppDemoIot(void)
{
    ...
    attr.priority = CN_IOT_TASK_PRIOR;  //优先级设定
